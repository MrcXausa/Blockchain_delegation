...
contract Delega {
    ...
    struct Services {
        //represent the services delegated to every single delegated
    ...
    function delegate(
        ...
        //emit LogDelegation(delegated, institution, service);
        ...
        users[msg.sender]
            .institutions[institution]
            .delegateds[delegated]
            .serviceIsPresent[hash(service)] = true;
        uint lengthServices = users[msg.sender]
            .institutions[institution]
            .delegateds[delegated]
            .services
            .length;
        users[msg.sender]
            .institutions[institution]
            .delegateds[delegated]
            .serviceIndex[service] = lengthServices - 1;

        if (
            !users[msg.sender].institutions[institution].addressIsPresent[
                delegated
            ]
        ) {
            ...
            users[msg.sender].institutions[institution].delegatedAddresses.push(
                delegated
            );
            uint addressesLength = users[msg.sender]
                .institutions[institution]
                .delegatedAddresses
                .length;
            users[msg.sender].institutions[institution].delegatedIndex[
                delegated
            ] = addressesLength - 1;
        }
    }
    ...
    function institutionDelegations(
        ...
        uint AddressesLength = users[user]
        ...
        for (uint i = 0; i < AddressesLength; i++) {
            address delegated = users[user]
                .institutions[msg.sender]
                .delegatedAddresses[i];
            returnValue memory sup;
            sup.delegated = delegated;
            sup.services = users[user]
                .institutions[msg.sender]
                .delegateds[delegated]
                .services;
            ret[i] = sup;
